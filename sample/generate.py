...
def main(args=None):
    ...
    if out_path == "":
        out_path = os.path.join(
            ...
            "cfg{}_len{}_samples_{}_{}_seed{}".format(str(args.guidance_param).replace(" ", "_").replace(".", ""), n_frames ,name, niter, args.seed),
        )
        if args.text_prompt != "":
    ...
    if args.text_prompt != "":
        ...
        assert (
            args.autoregressive
        ), "Dynamic text sampling is only supported with autoregressive sampling."
        with open(args.dynamic_text_path, "r") as fr:
            texts = fr.readlines()
        texts = [s.replace("\n", "") for s in texts]
        n_frames = (
            len(texts) * args.pred_len
        )  # each text prompt is for a single prediction
    ...
    if args.dynamic_text_path != "":
        ...
        if args.text_encoder_type == "bert":
            model_kwargs["y"]["text_embed"] = (
                model_kwargs["y"]["text_embed"][0]
                .unsqueeze(0)
                .repeat(args.num_samples, 1, 1, 1),
                model_kwargs["y"]["text_embed"][1]
                .unsqueeze(0)
    ...
    for rep_i in range(args.num_repetitions):
        ...
        sample = motion_temporal_filter(sample, sigma=1.5)
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            gt_frames = (
                np.arange(args.context_len)
                if args.context_len > 0 and not args.autoregressive
                else []
            )
...
def is_substr_in_list(substr, list_of_strs):
    return (
        np.char.find(list_of_strs, substr) != -1
    )  # [substr in string for string in list_of_strs]
...
def motion_temporal_filter(motion, sigma=1):
    ...
    motion_2d = motion.reshape(-1, T)

    for i in range(motion_2d.shape[0]):
        motion_2d[i] = gaussian_filter(motion_2d[i], sigma=sigma, mode="nearest")

    motion_filtered = motion_2d.reshape(B, J, C, T)
    return torch.from_numpy(motion_filtered)
