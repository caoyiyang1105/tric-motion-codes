...
class GaussianDiffusion:
    ...
    def plms_sample(
        ...
        if order > 1 and old_out is None:
            ...
            if cur_order == 1:
                ...
                eps_prime = (23 * old_eps[-1] - 16 *
                             old_eps[-2] + 5 * old_eps[-3]) / 12
            elif cur_order == 4:
                eps_prime = (
                    55 * old_eps[-1] - 59 * old_eps[-2] + 37 * old_eps[-3] - 9 * old_eps[-4]) / 24
            else:
        ...
        sample = mean_pred * nonzero_mask + \
            out["pred_xstart"] * (1 - nonzero_mask)
    ...
    def fc_loss_rot_repr(self, gt_xyz, pred_xyz, mask):
        ...
        gt_joint_xyz = gt_xyz[:, [l_ankle_idx,
                                  l_foot_idx, r_ankle_idx, r_foot_idx], :, :]
        gt_joint_vel = torch.linalg.norm(
            gt_joint_xyz[:, :, :, 1:] - gt_joint_xyz[:, :, :, :-1], axis=2)  # [BatchSize, 4, Frames]
        fc_mask = (gt_joint_vel <= 0.01)
        # [BatchSize, 4, 3, Frames]
        pred_joint_xyz = pred_xyz[:, [l_ankle_idx,
                                      l_foot_idx, r_ankle_idx, r_foot_idx], :, :]
        pred_joint_vel = torch.linalg.norm(
            pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1], axis=2)  # [BatchSize, 4, Frames]
        # Blank non-contact velocities frames. [BS,4,FRAMES]
    ...
    def foot_contact_loss_humanml3d(self, target, model_output):
        ...
        for chosen_vel_foot_calc, chosen_vel_foot, joint_idx, contact_mask_idx in zip(
                ...
                [7, 10, 8, 11],
                [0, 1, 2, 3]):
            tmp_mask_gt = contact_mask_gt[:, contact_mask_idx, :, :].cpu(
            ).detach().numpy().reshape(-1).astype(int)
    ...
    def velocity_consistency_loss_humanml3d(self, target, model_output):
        ...
        r_rot_quat_adapted = r_rot_quat[..., :-1, None,
                                        :].repeat((1, 1, 1, 21, 1)).to(calc_vel_from_xyz.device)
        print(
        ...
        calc_vel_from_xyz = motion_process.qrot(
            r_rot_quat_adapted, calc_vel_from_xyz)
        ...
        import matplotlib.pyplot as plt
        for i in range(21):
            plt.plot(np.linalg.norm(calc_vel_from_xyz[:, i*3:(i+1)*3, :, :].cpu(
            ).detach().numpy().reshape((3, -1)), axis=0), label='Calc Vel')
            plt.plot(np.linalg.norm(velocity_from_vector[:, i*3:(i+1)*3, :, :].cpu(
            ).detach().numpy().reshape((3, -1)), axis=0), label='Vector Vel')
...
# 1. 定义一个包装类 (如果你的 model forward 必须包含 kwargs)
class ModelWrapper(torch.nn.Module):
    ...
    def forward(self, x, t):
        # 这里模拟你原来的调用方式
        # 注意：你需要确保传入的 t 已经是 scale 过的，或者在这里做处理
        return self.model(x, t, **self.model_kwargs)